...
The docker volume plugin loads runtime options and values from a json configuration file (default `/etc/vsphere-storage-for-docker.conf`) on the host. The user can override the default configuration by providing a different configuration file, via the `--config` option, specifying the full path of the file. Options that are currently recognized include the below set. Options passed on the command line override those in the configuration file.
